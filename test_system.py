...
from chatbot.policy_engine import OrderStatus, PolicyEngine
...
def test_policy_engine():
    ...
    print(
        f"   Recent order cancellation: {'ALLOWED' if decision.allowed else 'DENIED'}"
    )
    ...
    decision = policy_engine.evaluate_cancellation(order_data)
    print(
        f"   Shipped order cancellation: {' ALLOWED' if decision.allowed else ' DENIED'}"
    )
...
def test_integration():
    ...
    print(
        f"   Policy decisions: {' SUCCESS' if (cancellable_count + approval_count + denied_count) == len(orders) else ' FAILED'}"
    )
...
def test_api_structure():
    ...
    try:
        ...
        routes_found = sum(
            1 for route in expected_routes if any(route in r for r in routes)
        )
