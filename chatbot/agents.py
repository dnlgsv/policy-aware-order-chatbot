...
from .order_service import order_service, Order
...
class IntentClassification(BaseModel):
    intent: Literal["order_cancellation", "order_tracking", "general_inquiry", "complaint", "unknown"]
    confidence: float
...
class RouterAgent(ChatbotAgent):
    ...
    def classify_intent(self, message: str, conversation_history: List[Dict[str, str]]) -> IntentClassification:
...
class CancellationAgent(ChatbotAgent):
    ...
    def process_cancellation_request(self, state: ChatState) -> Dict[str, Any]:
        ...
        if success:
            refund_timeline = self.policy_engine.order_policy.get_refund_timeline(order.status)
            return {
    ...
    def generate_response(self, state: ChatState, cancellation_result: Dict[str, Any]) -> str:
...
class TrackingAgent(ChatbotAgent):
    ...
    def generate_response(self, state: ChatState, tracking_result: Dict[str, Any]) -> str:
...
class PolicyAwareChatbot:
    ...
    def _route_conversation(self, state: dict) -> dict:
        ...
        intent_result = self.router.classify_intent(current_message, conversation_history)
