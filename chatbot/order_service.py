...
class OrderService:
    ...
    def _generate_sample_orders(self) -> Dict[str, Order]:
        ...
        for i in range(5):
            ...
            order = Order(
                ...
                status=random.choice([OrderStatus.PENDING.value, OrderStatus.PROCESSING.value]),
                items=[
                    {
                        "name": f"Product {i+1}",
        ...
        for i in range(5, 10):
            ...
            order = Order(
                ...
                customer_email=f"customer{i}@example.com",
                order_date=order_date.isoformat(),
                status=random.choice([OrderStatus.SHIPPED.value, OrderStatus.DELIVERED.value]),
                items=[
                    {
                        "name": f"Product {i+1}",
    ...
    def get_orders_by_email(self, email: str) -> List[Order]:
        ...
        return [order for order in self._orders.values() if order.customer_email == email]
    ...
    def get_tracking_info(self, order_id: str) -> Optional[TrackingInfo]:
        ...
        if order.status in [OrderStatus.PROCESSING.value, OrderStatus.SHIPPED.value, OrderStatus.DELIVERED.value]:
