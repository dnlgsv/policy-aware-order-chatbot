...
class ResponseQualityScore(BaseModel):
    ...
    score: float = Field(..., description="Numerical score from 1 (poor) to 5 (excellent) for the response quality.")
    justification: str = Field(..., description="Brief justification for the assigned score.")
    critique: Optional[str] = Field(default=None, description="Specific aspects the chatbot could improve.")
...
class ChatbotEvaluator:
    ...
    def _calculate_entity_accuracy(self, expected: Dict[str, str], predicted: Dict[str, Any]) -> float:
    ...
    def _evaluate_policy_compliance(self, test_case: TestCase, result: Dict[str, Any]) -> bool:
        ...
        if policy_decision_data:
            if isinstance(policy_decision_data, dict):
                ...
                actual_requires_approval = policy_decision_data.get("requires_approval", False)
            else:
            ...
            if test_case.expected_policy_outcome == "allowed":
                return (
                    ("cancelled" in response_text or "tracking information" in response_text) and
                    "approval" not in response_text and
                    "sorry" not in response_text and
                    "unable" not in response_text
                )
            elif test_case.expected_policy_outcome == "denied":
                return ("sorry" in response_text or 
                        "unable" in response_text or 
                        "cannot" in response_text or 
    ...
    async def _evaluate_response_quality(self, user_message: str, chatbot_response: str, test_case_context: Dict[str, Any]) -> float:
        ...
        try:
            evaluation = await chain.ainvoke({
                ...
                "test_case_context": json.dumps(test_case_context), # Ensure context is a string for the prompt
            ...
            logger.info(f"Response Quality Eval: Score={evaluation.score}, Justification='{evaluation.justification}', Critique='{evaluation.critique}'")
    ...
    async def run_evaluation(self) -> Dict[str, Any]:
        ...
        for i, test_case in enumerate(self.test_cases):
            print(f"Evaluating test case {i+1}/{len(self.test_cases)}: {test_case.id}")
    ...
    def _calculate_metrics(self, results: List[EvaluationResult]) -> Dict[str, Any]:
        ...
        policy_compliance_rate = sum(r.policy_compliance for r in successful_cases) / len(successful_cases) if successful_cases else 0
        
        # average response quality
        avg_response_quality = np.mean([r.response_quality_score for r in successful_cases]) if successful_cases else 0
    ...
    def _generate_report(self, results: List[EvaluationResult], metrics: Dict[str, Any]) -> str:
...
"""
    ...
    def save_results(self, evaluation_data: Dict[str, Any], output_dir: str = "evaluation_results"):
